...
Strategy.prototype.userProfile = function (accessToken, ticket, done) {
  ...
  this.userDetail(accessToken, ticket, function (err, json) {
...
Strategy.prototype._convertCodeToUser = function (accessToken, code, cb) {
  ...
  var ret = curl(this._profileURL + '?access_token=' + accessToken + '&code=' + code, {
    dataType: 'json',
    timeout: 5000
  });
...
Strategy.prototype.userDetail = function (accessToken, userTicket, done) {
  ...
  var ret = curl('https://qyapi.weixin.qq.com/cgi-bin/user/getuserdetail?access_token=' + accessToken,
    {
      dataType: 'json',
      timeout: 5000,
      method: 'POST',
